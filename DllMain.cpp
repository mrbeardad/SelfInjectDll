...
// #define TARGET_PROCESS_NAME "TargetProcessName.exe"
	...
	auto argv = ::CommandLineToArgvW(processName.c_str(), &size);
	if (argv)
	{
		processName = argv[0];
		::LocalFree(argv);
	...
	if (processName.ends_with(L".exe"))
	{
		...
		if (pid == 0)
		{
			DLOGW(L"ERROR: process {} not found\n", processName.c_str());
			return;
		}
	...
	{
		// Search the window by name
		auto& windowName = processName;
		HWND  hwnd = ::FindWindowW(NULL, windowName.c_str());
		if (!hwnd)
		{
			DLOGW(L"ERROR: window {} not found\n", windowName.c_str());
			return;
		}
		DWORD pid = 0;
		...
		if (tid)
		{
			::SetWindowsHookExW(WH_GETMESSAGE, (HOOKPROC)GetMsgProc, (HINSTANCE)&__ImageBase, tid);
			// Sleep for a while to insure there is enough time to inject dll
			for (int i = 0; i < 3; i++)
			{
